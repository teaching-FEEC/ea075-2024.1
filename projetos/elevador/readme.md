...
O projeto possui dois atuadores, ambos motores CC, para controle de abertura e fechamento das portas do elevador e para subir e descer a cabine. O motor utilizado para o deslocamento vertical da cabine apresenta bastante torque, portanto, sua potência será alta. Com isso, a alimentação é feita utilizando um retificador controlado com tiristores. Para controle do sentido e da velocidade do motor, será utilizada na saída do retificador uma ponte H com transistores MOSFET, conectada ao microcontrolador. Já para o motor CC da porta, é utilizada uma fonte de menor potência, e seu controle será feito por uma ponte H conectada ao microcontrolador.

O controle da parada correta da cabine do elevador é feito utilizando dois sensores magnéticos em cada andar, porém ainda não foi definido o modelo exato para o projeto. Assim, ao passar pelo sensor, o sistema saberá em que andar está e o momento correto para iniciar a frenagem. Outra importante função desse periférico é estacionar a cabine de forma extremamente precisa, alinhando o piso do elevador com o andar.
